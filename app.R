...
sidebar=dashboardSidebar(sidebarMenu(
                 ...
                 menuItem("Conclusion",tabname="conlusion")
))
body=dashboardBody(
  tabItems(
            ...
            fluidRow(
              tabsetPanel(
                tabPanel("Overview",
                         br(),
                         br(),
                         fluidRow( column(12,box(width = 8, title = "Pollution level and Revenue of a company", status = "primary", solidHeader = TRUE, background = "green",
                             fluidRow(
                           column (8, offset=1,
                                   sliderInput("pollution","Pollution Level", -500,1000,c(0,100)))
                         ),
                         fluidRow(column(12,plotOutput("reven_poll"))) ),
                         column(4, box(width=12,title="enter the name of industry present "))
                         ))
                                  ...
                                  tags$ul(
                                    ...
                                    tags$li("for analysis I have created three column :", tags$br(), tags$br(),
                                            tags$ul(tags$li("Industry_code: Based on International Standard Industrial Classification Revision 3.1 (ISIC)"), tags$br(),
                                                    tags$li(" pollution level : contain categorical value according to environment intensity like low for below 10% environment intensity etc."),tags$br(),
